...
export const getFullPath = (parentPath: string, name: string): string => path.join(parentPath, path.sep, name);
...
export const updateLicense = async (licensePath: string, licenseName: string) => {
  try {
    const license = getFullPath(licensePath, licenseName);
    const file = await fs.readFile(license);
    const string = `${file.toString()}`.replace(/(^Copyright.*)\s(\d+)\s(.*)/m, `$1 ${new Date().getFullYear()} $3`);
...
export const workspaceFolder = path.resolve(path.dirname(url.fileURLToPath(import.meta.url)), "../../..");
...
export const updatePackageJSON = async (packageJSONPath: string, packageJSONName: string) => {
  try {
    const packageJSON = getFullPath(packageJSONPath, packageJSONName);

    await updateJSON(packageJSON);

    const { stdout, stderr } = await exec(`pnpm exec prettier ${packageJSON} --write`, {
      cwd: packageJSONPath,
    });
...
const updateJSON = async (packageJSON: string) => {
  try {
    ...
    if (Array.isArray(data.extensionDependencies)) {
      data.extensionDependencies = normalizeIdentifiers(data.extensionDependencies);
    }
 ...
 */
const normalizeIdentifiers = (identifiers: string[]) => identifiers.map((identifier) => identifier.toLowerCase());
